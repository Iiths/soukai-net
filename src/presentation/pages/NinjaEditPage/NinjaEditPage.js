...
function ninjaToForm(ninja, episodeMap) {
...
function formToNinja(form) {
    ...
    const episodeRefs = appearances.map((ep) => ({ id: ep.id }));
    return {
        ...ninjaFields,
...
export function NinjaEditPage() {
    ...
    const addEpisode = (ep) => {
        if (form?.appearances.some(e => e.id === ep.id))
            return; // 重複追加防止
    ...
    const filteredEpisodes = episodeSearch.trim()
        ? allEpisodes.filter(ep => ep.title.toLowerCase().includes(episodeSearch.toLowerCase()) ||
            ep.id.includes(episodeSearch)).slice(0, 10)
        : [];
    const handleSave = () => {
        if (!form)
            return;
